...
import { LANGUAGE_CATALOG, findLanguage } from '../lib/languages.js';
 ...
 * on English is not the same as forcing English — the backend still
 * detects a Devanagari or Kannada query and answers in kind. Picking one
 * only removes the guess, which matters because the detector is a script
 * heuristic and cannot tell Hindi from Marathi.
 *
 * A dropdown rather than one button per language: LANGUAGE_CATALOG covers
 * English plus all 22 scheduled languages, and 23 header buttons would
 * not fit any screen. Searchable because scrolling a 23-item list to find
 * one entry is worse than typing three letters of it.
 */
export function LanguageDropdown({ value, onChange }) {
  ...
  const filtered = q
    ? LANGUAGE_CATALOG.filter(l =>
        l.englishName.toLowerCase().includes(q) || l.nativeName.toLowerCase().includes(q))
    : LANGUAGE_CATALOG;

  return (
    <div className="lang lang-dropdown" ref={rootRef}>
      <button
        type="button"
        className="lang-opt lang-dropdown-trigger"
      ...
      {open && (
        <div className="lang-dropdown-panel" role="listbox" aria-label="Answer language">
          <input
            type="text"
          ...
          <div className="lang-dropdown-list">
            {filtered.map(l => (
              ...
              >
                ...
                {l.code !== null && <span className="lang-english"> — {l.englishName}</span>}
              </button>
            ))}
            {filtered.length === 0 && (
              <div className="lang-dropdown-empty">No matching language</div>
            )}
          </div>
        </div>
      )}
...
/**
 * Backwards-compatible alias — see the Confidence/EvidenceStatus note
 * above. `LanguageToggle` was the name from when this was three buttons;
 * it is a searchable dropdown over 23 languages now, and the spec names
 * it LanguageDropdown.
 */
export const LanguageToggle = LanguageDropdown;
